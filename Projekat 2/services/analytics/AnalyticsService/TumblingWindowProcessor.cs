...
public class TumblingWindowProcessor : IAsyncDisposable
{
        ...
        try
        {
            while (await timer.WaitForNextTickAsync(ct))
            {
                ...
                _logger.LogInformation(
                    "Prozor zatvoren [{Start:HH:mm:ss}-{End:HH:mm:ss}]: poruke={Tot} avg={Avg:F2}C e2e={E2E:F0}ms alert={Al} broker={B}",
                    winStart.LocalDateTime, winEnd.LocalDateTime, msgCount, avgTemp, avgE2eMs, alert, _broker);

                _ = Task.Run(() => SaveWindowAsync(winStart, winEnd, msgCount, avgTemp, maxTemp, minTemp, avgHum, alert, avgE2eMs), CancellationToken.None);
            }
        }
    ...
    private async Task SaveWindowAsync(DateTimeOffset winStart, DateTimeOffset winEnd,
        ...
        bool alertTriggered, double avgE2eMs)
    {
        const string sql = @"
