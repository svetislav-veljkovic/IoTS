...
const csvData = new SharedArray('Real IoT Data Devices for Test B', function () {
...
export default () => {
    ...
    const restRes = http.get(`http://localhost:5001/api/SensorData/aggregated/${realDeviceId}?days=1`);
    ...
    const gqlQuery = `
        query {
            getLatestReadings(deviceId: "${realDeviceId}", limit: 1) {
                temperature
                humidity
            }
        }
